...
const gsession_t = getCookie(SESSIONKEY);
const scannedData_t = [ ];
...
+async function(){
    ...
    $.post("/org/manage/auth/editor", { session: gsession_t, type: "Ticket" })
    .then(editorData => {
        ...
        async function startScanningStamp(){
            ...
            const foundQRCode = (data) => {
                ...
                $.post("/org/manage/ticket/scanned", { data: data })
                .then((data) => {
                    /**@type {string} */
                    const msg = data.message;
                    /**@type {Ticket} */
                    const used = data._used;

                    PictoNotifier.notifySuccess(`チケットを確認できました: ${used.visual.description} => ${rl.user_id}`);
                })
                .catch(e => {
                    //@ts-ignore
                    PictoNotifier.notifyError(`非合法なチケットでした: ${e.message}`);
                })
            ...
            const checkImage = async () => {
                ...
                if (code && code.data){
                    ...
                    if (scannedData_t.includes(strdata)){
                    ...
                    scannedData_t.push(strdata);
                    setTimeout(() => {
                        scannedData_t.splice(scannedData_t.indexOf(strdata), 1);
                    }, (60)*1000);
                    return;
                } else {
