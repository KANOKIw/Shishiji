...
/**
 * @typedef {import("../shishiji-dts/objects").mapObject} _mapObject
*/

const secReg = /(§[a-zA-Z0-9]){1}/g;
const session = getCookie("__ogauthk");
var username = "";
/**@ts-ignore @type {_mapObject} */
var ARTICLEDATA = {};
var change_not_saved_remaining = false;
const orgCloudfi = {
    maxsize: 0,
};
const SETTINGS = {
...
const accepted_cloudfileExtensions = [".jpg", ".jpeg", ".png", ".gif", ".webp", ".mp4", ".webm", ".avi", ".mov", ".flv"];
...
function colorXtext(text){
    ...
    text = text.replace(/(#\:LINK)-H=(https?:\/\/(?!.*#:).*)-T=(.*)(;#)/g, `<span class="sheart24">$1-H=<span class="sheart25">$2</span>-T=<span class="sheart25">$3</span>$4</span>`);
...
window.addEventListener("load", function(e){
    ...
    this.document.getElementById("smthElse")?.addEventListener("click", function(e){
        ...
        <div class="protected aioshud" id="ppupds" style="display: flex; align-items: center; justify-content: center; flex-flow: column;">
            ...
            <h4>フォント; (規定値: sans-serif)</h4>
            <input type="text" id="font_family_input" value="${ARTICLEDATA.article.font_family}" placeholder="font-family: ?;">
            <hr class="dhr-ppo rsgafwad">
            <h4>中心学年</h4>
        ...
        `, () => {
            for (const op of document.querySelectorAll("option")){
                if (op.value === ARTICLEDATA.article.core_grade){
                    op.selected = true;
                }
            }
            /**@ts-ignore @type {{[key: string]: NodeJS.Timeout}} */
        ...
        }, false, { width: 500, height: 550, });
    });


    this.document.getElementById("__undo_-")?.addEventListener("click", () => {
        document.execCommand("undo", false);
    });


    this.document.getElementById("__redo_-")?.addEventListener("click", () => {
...
window.addEventListener("beforeunload", e => {
    if (change_not_saved_remaining || (Popup.popupping && $("#ppupds").hasClass("aioshud"))){
        e.preventDefault();
