...
try:
    ...
    sys.exit(f"{Fore.RED}Couldn't find {jar_path}, must be a relative path")
...
async def on_ready():
    ...
    print(f"{Fore.RED}to terminate, please kill {jar_path}.{Fore.RESET}")
    ...
    async def run_jar():
        ...
        print(f"{Fore.RED}{jar_path} teminated.{Fore.RESET}")
    ...
    async def console_log():
        ...
        previous_content = ""
        last_log_message = await channel.send(toDiff(f"+ New server created at: {current_time(year=True)}\n+ -----------------------..."))
            ...
            try:
                with open(config_path, "r") as f:
                    ...
                    if console_channel_id != int(config["console_channel_id"]):
                        ...
                        channel = client.get_channel(console_channel_id)
                        last_log_message = await channel.send(toDiff(f"+ Changed console channel from {previous_channel.guild.name}/#{previous_channel.name}: {current_time(year=True)}\n+ -----------------------..."))

                with open("./logs/latest.log", "r", encoding="Shift_JIS") as f:
                    content = f.read()
                    if content != previous_content and previous_content in content:   
                        last_log_message_content = last_log_message.content
                        _content = content.replace((previous_content), "").strip() 
                        _content = log_into_diff_decorator(log=_content, console=True)
                        _content = last_log_message_content[7:-3] + _content   
                        repeat = int(len(_content)/1980) +2                      
                        if repeat <= 2: last_log_message = await last_log_message.edit(content=toDiff(_content))
                        else:
                            for num in range(1, repeat):
                                ...
                                if num == 1:
                                    last_log_message = await last_log_message.edit(content=toDiff(message_content))
                                else:
                                    last_log_message = await channel.send(content=toDiff(message_content))
                    previous_content = content
            except Exception as e: print(e)

            await asyncio.sleep(1)

    jar_process = asyncio.create_task(run_jar())
    consoleloop = asyncio.create_task(console_log())
    exitloop = asyncio.create_task(exit_self())
    await asyncio.wait_for(jar_process, timeout=None)
    consoleloop.cancel()
    await before_exit()
